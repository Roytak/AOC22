...
unsigned long long
find_smallest(struct directory *tree, unsigned long long required_space, unsigned long long *min)
{
	unsigned long long sum = 0;
	...
	sum += find_smallest(tree->child, required_space, min);

	sum += tree->file_size;

	if (sum >= required_space) {
		if (sum < *min) {
			*min = sum;
		}
	}

	sum += find_smallest(tree->next, required_space, min);
...
main(void)
{
	...
	char buffer[BUFSIZE];
	unsigned long long sum = 0;
	unsigned long long req_space = 0;
	unsigned long long ans = 70000000;
	...
	tree_sum(home, &sum);
	req_space = 30000000 - (70000000  - sum);
	printf("%llu\n", req_space);
	find_smallest(home, req_space, &ans);
	printf("ans: %llu\n", ans);
