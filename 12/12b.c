...
enqueue(struct queue *q, struct node *n)
{
	...
	if (!q->front) {
		...
		if (dupl_open(q, n) || ance_open(n)) {
			printf("Node [%d, %d, %d] is invalid.\n", n->x, n->y, n->steps);
			free(n);
			free(qn);
			return;
		}

		if (q->front->data->steps >= n->steps) {
			qn->next = q->front;
			q->front = qn;
			return;
		}

		for (iter = q->front; iter->next; iter = iter->next) {
			if (iter->next->data->steps > n->steps) {
				break;
			}
		}
		qn->next = iter->next;
...
bfs(struct queue *q)
{
	...
	while (buf[curr->x][curr->y] != 'E') {
		printf("Current node: [%d, %d, %d] = '%c'.\n", curr->x, curr->y, curr->steps, buf[curr->x][curr->y]);
		enqueue_next(q, curr);
		free(curr);
		curr = dequeue(q);
	}
